...
Said guide had a beautiful map that placed attacker IP's into world map but I found out that the geoip list provided was incorrect and had conflicting duplicates. I changed the watchlist with KQL's built-in `geo\\\\\\\\\\\\\\\_info\\\\\\\\\\\\\\\_from\\\\\\\\\\\\\\\_ip\\\\\\\\\\\\\\\_address()`,
...
!\[Attack maps Screenshot](images/0\_map\_watchlist.png)
...
!\[Failed logon counts by source IP](images/2\_IP\_Attacker\_List.png)
...
| extend geo = geo\\\\\\\\\\\\\\\_info\\\\\\\\\\\\\\\_from\\\\\\\\\\\\\\\_ip\\\\\\\\\\\\\\\_address(IpAddress)
...
| project FailureCount, AttackerIp = IpAddress, latitude, longitude, city, country,
          friendly\\\\\\\\\\\\\\\_location = strcat(city, " (", country, ")")
```
...
!\[Closed incident: Benign Positive classification with comment](images/3\_Triage\_Benign.png)
